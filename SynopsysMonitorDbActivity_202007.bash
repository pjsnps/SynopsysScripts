...
#VERSION:    ____         # :! date -u +\%y\%m\%d\%H\%MZ
#GREPVCKSUM: ____ under dev# :! grep -v grepvcksum <script> | cksum
#CHANGELOG:  ___ pj Add sleep command line positional parameter $1.  Add advanced usage, with new version of SnpsDbStatDelta_202008.bash which accepts input via pipe from output of this script. Change shebang to /usr/bin/bash.
 
#PURPOSE:    To try to detect database activity during long silent software upgrades, database migrations, etc.  
#PURPOSE:    Meant to only be used maybe every hour or two during an event, just to make sure the database is still processing data silently. 

#REQUIRES:  psql, Black Duck server.
#RECOMMENDED:  ./SnpsDbStatDelta_202008.bash to calculate Deltas between two runs of stats commands. 
...
Can use any one (only one for now) of these "cmd" in grep -zPo filter:
...
Just show the deltas for bds_hub db:
