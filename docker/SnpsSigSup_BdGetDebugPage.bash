#!/usr/bin/bash
#SCRIPT: SnpsSigSup_BdGetDebugPage.bash
...
#[pjalajas@sup-pjalajas-hub docker]$ cat SnpsSigSup_BdGetDebugPage.bash | bash | wc
...
#[pjalajas@sup-pjalajas-hub docker]$ cat SnpsSigSup_BdGetDebugPage.bash | bash | grep -Pzo '(?s)Current Queries.*?\n\n'
#To remove trailing spacies from queries output:
#[pjalajas@sup-pjalajas-hub docker]$ cat SnpsSigSup_BdGetDebugPage.bash | bash | grep -Pzo '(?s)Current Queries.*?\n\n' | sed -re 's/ *$//' | cat -A | less -inRF


#[pjalajas@sup-pjalajas-hub docker]$ bash --rpm-requires SnpsSigSup_BdGetDebugPage.bash | sort -u
#executable(bash)
#executable(cat)
